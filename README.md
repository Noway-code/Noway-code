# Hi, I'm Camilo! 👋

---
## About Me
- 🔬 **Undergraduate Researcher** @ ADC Lab - UCF
- 🔭 **Ex-Intern Software Developer** @ Thermo Fisher Scientific
- 📚 CS Major, Minor in Mathematics
- 🏋️‍♂️ Bodybuilder & Future Marathoner
- 📖 Check out my [Reads on StoryGraph](https://app.thestorygraph.com/profile/nowayreads)

---

## Languages & Tools

<div align="center">
  <img src="https://cdn.simpleicons.org/archlinux/1793d1" height="40" alt="Arch" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" height="40" alt="C" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="40" alt="C#" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="40" alt="Java" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="40" alt="HTML5" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="40" alt="CSS3" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="40" alt="Python" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="40" alt="React" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="40" alt="Node.js" />
  <img src="https://skillicons.dev/icons?i=tailwind" height="40" alt="Tailwind CSS" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/webstorm/webstorm-original.svg" height="40" alt="WebStorm" />
</div>

---

## My Stats

<div align="center">
  <img src="https://streak-stats.demolab.com?user=noway-code&locale=en&mode=daily&theme=nord&hide_border=false&border_radius=5&order=3" height="175" alt="Streak Stats" />
</div>

<div align="center">
  <div style="display: flex; justify-content: center; align-items: flex-start; gap: 20px;">
    <div>
      <p><strong>Top Languages</strong></p>
      <img src="https://github-readme-stats.vercel.app/api/top-langs?username=noway-code&locale=en&hide_title=true&layout=donut-vertical&card_width=320&langs_count=6&theme=nord&hide_border=false&order=2&hide=shell,html,css,lua" height="175" alt="Languages" />
    </div>
    <div>
      <p><strong>Top Scripts</strong></p>
      <img src="https://github-readme-stats.vercel.app/api/top-langs?username=noway-code&locale=en&hide_title=true&layout=donut-vertical&card_width=320&langs_count=6&theme=nord&hide_border=false&order=2&hide=c,javascript,typescript,svelte" height="175" alt="Scripts" />
    </div>
  </div>
</div>

---

## Projects

### [TypeAlong](https://github.com/noway-code/TypeAlongSvelte) 📖
A sleek, minimalistic typing game built with SvelteKit and TypeScript. It transforms your fave EPUB books into fun, immersive typing challenges.

### [Ladybug](https://github.com/LadyBugML/ladybug) 🐞
A GitHub bot that aggregates GUI info and bug reports. It flexes some deep learning magic to help you pinpoint bugs in your repos.

### [Nowaycode.com](https://github.com/Noway-code/timetocook) 🌐
My personal hub for work, blogging, and web projects—a central landing page for all the cool stuff I do.

---

## Let's Connect
Feel free to hit me up on [GitHub](https://github.com/noway-code) or drop a message if you wanna collab or just chat tech. Stay awesome!